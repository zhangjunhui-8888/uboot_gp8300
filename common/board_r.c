...
#endif
#ifdef CONFIG_REINIT_GLOBAL_DATA
  void dlmalloc_reinit_vars(void);
  void env_reset_env_id(void);
  void env_reinit_vars(void);
  void net_reinit_vars(void);
  void env_flags_reinit_vars(void);
  void env_callback_reinit_vars(void);
  dlmalloc_reinit_vars();
  env_reset_env_id();
  env_reinit_vars();
  net_reinit_vars();
  env_flags_reinit_vars();
  env_callback_reinit_vars();
#endif
	return 0;
}
...
#ifdef CONFIG_SHOW_LOGO_GPT
static int initr_gpt_image(void)
{
	run_command_list(getenv("fill_logo"), -1, 0);
	return 0;
}
#endif

#ifdef CONFIG_VIDEO_GPT
static int initr_gpt_video(void)
{
	run_command_list(getenv("init_vdo"), -1, 0);
	return 0;
}
#endif
...
{
#if defined(CONFIG_GPT_WATCHDOG)
	unsigned long wdt_timeout = getenv_ulong("wdt_timeout", 10, 4294967295);
	if (wdt_timeout != 4294967295){
		hw_watchdog_settimeout(wdt_timeout);	
	}
#endif
...
#endif
	initr_env,
#if defined(CONFIG_VIDEO_GPT)
	initr_gpt_video,
#endif
#if defined(CONFIG_SHOW_LOGO_GPT)
	initr_gpt_image,
#endif
#ifdef CONFIG_SYS_BOOTPARAMS_LEN
