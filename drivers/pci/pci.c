...
int pci_hose_scan_bus(struct pci_controller *hose, int bus)
{
	...
	unsigned short vendor, device, class,  device_type;
	...
	for (dev =  PCI_BDF(bus,0,0);
		...
		if (vendor == 0xffff || vendor == 0x0000)
			continue;
		/*upd720201 pcie speed fix 2.5G */
		if (vendor == 0x10b5 || bus == 0x02 ) {
			pci_hose_write_config_dword(hose, dev, 0x98, 0x1);
			pci_hose_write_config_dword(hose, dev, 0x78, 0x20);
		}
		if (!PCI_FUNC(dev))
			found_multi = header_type & 0x80;
                ...
                pcie_cap_pos +=0x2;
                pci_hose_read_config_word(hose, dev, pcie_cap_pos, &device_type);
                /*root, EP and switch upstream port only support dev-0*/
                if (((device_type & 0xf0) >> 0x4 == 0x5 || (device_type & 0xf0) >> 0x4 == 0x4 ||
                         (device_type & 0xf0) >> 0x4 == 0x0) &&  (PCI_DEV(dev) != 0))
                        continue;
