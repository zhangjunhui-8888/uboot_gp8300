...
void gpt_pcie_rcmsi(struct pci_controller *hose)
{
#if 0
	u32 addr1 = (u32)GPT_RC_MSI_ADDR;
	u32 addr2 = GPT_RC_MSI_ADDR >> 32;
#else
	u32 addr1 = 0x30000000;
	u32 addr2 = 0x2;
#endif
	struct gpt_pcie *pcie = hose->priv_data;

	/* address */
	writel(addr1, pcie->regs + GPT_PCIE_MSI_ELADDR);
	writel(addr2, pcie->regs + GPT_PCIE_MSI_EHADDR);

	/* target */
	writel(GPT_RC_MSI_DATA, pcie->regs + GPT_PCIE_MSI_EDATA);

	/* enable */
	writel(0xffffffff, pcie->regs + GPT_PCIE_MSI_IRQ_EN); //???

}

static void gpt_pcie_rccfg(struct pci_controller *hose)
{
	u32 data;
	u32 reg;
	u32 addr1 = (u32)GPT_RC_BAR0_ADDR;
	u32 addr2 = (u32)(GPT_RC_BAR0_ADDR >> 32);
	struct gpt_pcie *pcie = hose->priv_data;
	...
	writel(data | 0x7, pcie->dbi + GPT_PCIE_RE_CMDST);
        ...
        writel(reg | (PCI_CLASS_BRIDGE_PCI << 16), pcie->dbi + 0x2000 + GPT_PCIE_FUNC_CLASS);
        reg = readl(pcie->dbi + 0x2000 +  GPT_PCIE_FUNC_CLASS);

	gpt_pcie_outbound_set(pcie, 0, GPT_PCIERE_REGN_CFG,
			GPT_OUT_DESC_TP_CFG0, GPT_PCIE_CFG0_BASE,
			GPT_PCIE_CFG0_BASE, 28);

	gpt_pcie_outbound_set(pcie, 0, GPT_PCIERE_REGN_IO,
			GPT_OUT_DESC_TP_IO, GPT_PCIE_IO_BASE,
			GPT_PCIE_IO_BASE, 28);
	...
	/* config bars */
	data = GPT_RC_BAR0_ENABLE(GPT_RC_BAR_MEM32, GPT_RC_BAR_4G)
		| GPT_RC_BAR1_ENABLE(GPT_RC_BAR_DISABLE, GPT_RC_BAR_4G)
		| (0x1 << 19)
		| GPT_RC_CHECK_EN;
	writel(data, pcie->dbi + GPT_PCIE_RC_BAR_CFG);

	writel(addr1, pcie->dbi + GPT_PCIE_BAR0);
	writel(addr2, pcie->dbi + GPT_PCIE_BAR1);

	gpt_pcie_rp_inbound_set(pcie, 0, 0x200000000, 32);
...
void gpt_pcie_rcmode(struct gpt_pcie *pcie)
{
	unsigned int state;
	state = readl(pcie->regs + GPT_PCIE_CFG);
	writel(state | GPT_PCIE_RCMD, pcie->regs + GPT_PCIE_CFG);
...
int gpt_pcie_init(void)
{
	...
	/* outbound i/o */
	pci_set_region(&(pcie->hose.regions[0]),
			(pci_size_t)GPT_PCIE_IO_BASE,
			(phys_size_t)GPT_PCIE_OTBND_BASE + GPT_PCIE_IO_BASE,
			(pci_size_t)GPT_PCIE_IO_SIZE,
			PCI_REGION_IO);

	/* outbound memory */
	pci_set_region(&(pcie->hose.regions[1]),
	...
	pci_set_region(&(pcie->hose.regions[2]),
			(pci_addr_t)GPT_PCIE_RC_SYSMEM,
			(phys_addr_t)GPT_PCIE_RC_SYSMEM,
			GPT_PCIE_RC_SYS_SIZE,
			PCI_REGION_SYS_MEMORY);

	pcie->hose.region_count = 3;
