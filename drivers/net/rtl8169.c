...
static int rtl_setup_mac(struct eth_device *dev)
{
	unsigned char enetaddr[6], env_enetaddr[6];
	int i;
	char *tmp, *end;
	ioaddr = (unsigned long) dev->iobase;
	...
	if(memcmp(enetaddr, env_enetaddr, 6)) {
		printf("\n%s, MAC address don`t match\n", __FUNCTION__);
		memcpy(enetaddr, env_enetaddr, 6);

		RTL_W32(MAC0 + 4, env_enetaddr[4] | env_enetaddr[5] << 8);
		RTL_W32(MAC0, env_enetaddr[0] | env_enetaddr[1] << 8 | env_enetaddr[2] << 16 | env_enetaddr[3] << 24);
		printf("MAC Addr");
		for (i = 0; i < MAC_ADDR_LEN; i++)
			enetaddr[i] = RTL_R8(MAC0 + i);
...
int rtl8169_initialize(bd_t *bis)
{
	...
	while(1){
		...
		dev->iobase = pci_mem_to_phys(devno, iobase);

		dev->init = rtl_reset;
		dev->halt = rtl_halt;
		dev->send = rtl_send;
		dev->recv = rtl_recv;
		dev->write_hwaddr = rtl_setup_mac;
