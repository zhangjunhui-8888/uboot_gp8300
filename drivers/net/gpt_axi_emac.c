...
struct axidma_priv {
	...
	int phyaddr;
#ifdef CONFIG_PHY_INTERNAL_ADDR
	int phyInaddr;
#endif
...
/* BD descriptors */
struct axidma_bd {
	u64 next;	/* Next descriptor pointer */
	u64 phys;	/* Buffer address */
	u32 reserved3;
	u32 reserved4;
	u32 cntrl;	/* Control */
	u32 status;	/* Status */
...
struct axi_regs {
	...
	u32 fmi;  /* 0x708: Filter Mask Index Register */
	u32 reserved7;              /* 0x70c */
	u32 af0;  /* 0x710: Address Filter Register 0 */
	u32 af1;  /* 0x714: Address Filter Register 1 */
};
...
static inline int mdio_wait(struct eth_device *dev)
{
	...
	while (timeout && (!(in_le32(&regs->mdio_mcr)
								...
								u16 *val)
{
	...
	out_le32(&regs->mdio_mcr, mdioctrlreg);

	if (mdio_wait(dev))
		return 1;

	/* Read data */
	*val = in_le32(&regs->mdio_mrd);
	return 0;
}
...
#ifdef CONFIG_PHY_INTERNAL_ADDR
	phyIndev = phy_connect(priv->bus, priv->phyInaddr, dev, 0);
  if (!phyIndev) {
		printf("axiemac: could not find internal PHY addr: %d!\n", priv->phyInaddr);
		return 0;
  }
#endif
	...
	priv->phydev = phydev;
#ifndef CONFIG_PHY_NO_CONFIG
	phy_config(phydev);
#else
  debug("axiemac: prepare aneg!\r\n");
...
#ifdef CONFIG_PHY_INTERNAL_ADDR
  phyIndev->speed = phydev->speed;
  phyIndev->duplex = phydev->duplex;
	/* phyIndev->supported = phydev->supported; */
	/* phyIndev->advertising = phydev->supported; */
  genphy_config_aneg(phyIndev);
	if (phy_startup(phyIndev)) {
		printf("axiemac: could not initialize PHY %s\n",
		       phyIndev->dev->name);
		return 0;
	}
	...
	emmc_reg = in_le32(&regs->emmc);
	emmc_reg &= ~XAE_EMMC_LINKSPEED_MASK;
	emmc_reg |= speed;

	/* Write new speed setting out to Axi Ethernet */
	out_le32(&regs->emmc, emmc_reg);
...
static void axiemac_halt(struct eth_device *dev)
{
	...
	/* Stop the hardware */
	temp = in_le32(&priv->dmatx->control);
	temp &= ~XAXIDMA_CR_RUNSTOP_MASK;
	out_le32(&priv->dmatx->control, temp);

	temp = in_le32(&priv->dmarx->control);
	temp &= ~XAXIDMA_CR_RUNSTOP_MASK;
	out_le32(&priv->dmarx->control, temp);
...
static int axi_ethernet_init(struct eth_device *dev)
{
	 ...
	 */
	while (timeout && (!(in_le32(&regs->is) & XAE_INT_MGTRDY_MASK))) {
		timeout--;
		udelay(1);
	}
	if (!timeout) {
	...
	out_le32(&regs->rcw1, in_le32(&regs->rcw1) & ~XAE_RCW1_RX_MASK);
	...
	out_le32(&regs->is, XAE_INT_RXRJECT_MASK);

	/* Setup HW */
	/* Set default MDIO divisor */
	out_le32(&regs->mdio_mc, XAE_MDIO_DIV_DFT | XAE_MDIO_MC_MDIOEN_MASK);
...
static int axiemac_setup_mac(struct eth_device *dev)
{
	...
	out_le32(&regs->uaw0, val);

	val = (dev->enetaddr[5] << 8) | dev->enetaddr[4] ;
	val |= in_le32(&regs->uaw1) & ~XAE_UAW1_UNICASTADDR_MASK;
	out_le32(&regs->uaw1, val);
	return 0;
}
...
static void axi_dma_init(struct eth_device *dev)
{
	...
	out_le32(&priv->dmatx->control, XAXIDMA_CR_RESET_MASK);
	out_le32(&priv->dmarx->control, XAXIDMA_CR_RESET_MASK);

	/* At the initialization time, hardware should finish reset quickly */
	while (timeout--) {
		/* Check transmit/receive channel */
		/* Reset is done when the reset bit is low */
		if (!((in_le32(&priv->dmatx->control) |
				in_le32(&priv->dmarx->control))
...
static int axiemac_init(struct eth_device *dev, bd_t * bis)
{
	...
	struct axi_regs *regs = (struct axi_regs *)dev->iobase;
	u32 temp;
	struct axidma_bd* rx=&rx_bd;
//	struct axidma_bd* tx=&tx_bd;
	...
	out_le32(&priv->dmarx->current, (u32)((u64)rx));
	out_le32(&priv->dmarx->reserved1,(u32)(((u64)rx)>>32));
	/* Setup the BD. */
	memset(&rx_bd, 0, sizeof(rx_bd));
	rx_bd.next = (u64)&rx_bd;
	rx_bd.phys = (u64)&rxframe;
	rx_bd.cntrl = sizeof(rxframe);
	/* Flush the last BD so DMA core could see the updates */
	flush_cache((unsigned long)&rx_bd, sizeof(rx_bd));

	/* It is necessary to flush rxframe because if you don't do it
	 * then cache can contain uninitialized data */
	flush_cache((unsigned long)&rxframe, sizeof(rxframe));
	...
	out_le32(&priv->dmarx->tail, (u32)((u64)rx));
	out_le32(&priv->dmarx->reserved2, (u32)(((u64)rx)>>32));
	/* Enable TX */
	out_le32(&regs->tc, XAE_TC_TX_MASK);
	/* Enable RX */
	out_le32(&regs->rcw1, XAE_RCW1_RX_MASK);
	...
	debug("axiemac: regs %lx dma %p Init complete\n",(unsigned long)dev->iobase,dev->priv);
	return 0;
}
...
static int axiemac_send(struct eth_device *dev, void *ptr, int len)
{
	...
	flush_cache((unsigned long)ptr, len);

	/* Setup Tx BD */
	memset(&tx_bd, 0, sizeof(tx_bd));
	/* At the end of the ring, link the last BD back to the top */
	tx_bd.next = (u64)&tx_bd;
	tx_bd.phys = (u64)ptr;
	/* Save len */
	...
	/* Flush the last BD so DMA core could see the updates */
	flush_cache((unsigned long)&tx_bd, sizeof(tx_bd));

	if (in_le32(&priv->dmatx->status) & XAXIDMA_HALTED_MASK) {
		u32 temp;
		out_le32(&priv->dmatx->current, (u32)((u64)tx));
		out_le32(&priv->dmatx->reserved1, (u32)(((u64)tx)>>32));
		/* Start the hardware */
		temp = in_le32(&priv->dmatx->control);
		temp |= XAXIDMA_CR_RUNSTOP_MASK;
		out_le32(&priv->dmatx->control, temp);
	...
	/* Start transfer */
	out_le32(&priv->dmatx->tail, (u32)((u64)tx));
	out_le32(&priv->dmatx->reserved2, (u32)(((u64)tx)>>32));
	/* Wait for transmission to complete */
#ifdef DEBUG_TX
	debug("axiemac: Waiting for tx[%p %p][%x %x] sr[%x] to be done\n", \
		&priv->dmatx->tail,&priv->dmatx->reserved2, \
		in_le32(&priv->dmatx->tail),in_le32(&priv->dmatx->reserved2),in_le32(&priv->dmatx->status));
#endif
	timeout = 200;
	while (timeout && ((!in_le32(&priv->dmatx->status)) &
...
static int isrxready(struct eth_device *dev)
{
	...
	status = in_le32(&priv->dmarx->status);

	/* Acknowledge pending interrupts */
	out_le32(&priv->dmarx->status, status & XAXIDMA_IRQ_ALL_MASK);
...
#ifdef CONFIG_PHY_INTERNAL_ADDR
	priv->phyInaddr = CONFIG_PHY_INTERNAL_ADDR;
#endif
	printf("axiemac init: base %lx dmatx %p dmarx %p phy %x\n", base_addr, priv->dmatx, priv->dmarx, CONFIG_PHY_ADDR);
