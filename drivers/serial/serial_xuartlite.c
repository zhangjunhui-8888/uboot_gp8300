...
static struct uartlite *userial_ports[4] = {
	...
	#define uart_write32(addr,val)	writel(val,addr)
	#define uart_read32(addr)	readl(addr)	
#else
	#define uart_write32(addr,val)	out_be32(val,addr)
	#define uart_read32(addr)	in_be32(addr)		
#endif
static void uartlite_serial_putc(const char c, const int port)
{
	...
	while (uart_read32(&regs->status) & SR_TX_FIFO_FULL)
		;
	uart_write32(&regs->tx_fifo, c & 0xff);
}
...
static int uartlite_serial_getc(const int port)
{
	...
	while (!(uart_read32(&regs->status) & SR_RX_FIFO_VALID_DATA))
		;
	return uart_read32(&regs->rx_fifo) & 0xff;
}
...
static int uartlite_serial_tstc(const int port)
{
	...
	return uart_read32(&regs->status) & SR_RX_FIFO_VALID_DATA;
}
...
static int uartlite_serial_init(const int port)
{
	...
	if (regs) {
		uart_write32(&regs->control, 0);
		uart_write32(&regs->control,
			 ULITE_CONTROL_RST_RX | ULITE_CONTROL_RST_TX);
		uart_read32(&regs->control);
